...
function convertHandler(dataRate) {
    ...
    amountInputOne.addEventListener('input', () => {
        amountInputTwo.value = (amountInputOne.value * dataRate[currencyTwo.value] / dataRate[currencyOne.value]).toFixed(2);
    })
    amountInputTwo.addEventListener('input', () => {
        amountInputOne.value = (amountInputTwo.value * dataRate[currencyOne.value] / dataRate[currencyTwo.value]).toFixed(2);
    })
    // On Selcet change
